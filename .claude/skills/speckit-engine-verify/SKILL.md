...
metadata:
  author: snackbyte
  source: engine:commands/speckit.engine.verify.md
---
...
1. **Recursive code review** — invoke the consolidated review module with target `code`:
   ...
   in a tested `.specify/extensions/engine/scripts/bash/*.sh`). It fixes the unambiguous issues each pass and returns
