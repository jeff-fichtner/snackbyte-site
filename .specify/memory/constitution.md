...
This is the public face of snackbyte and the first consumer of its brand. Two
bodies of law already govern it and are bound by reference, not restated here —
see Principle VIII.
...
`docs/BRAND.md` is the record of the brand: tokens, geometry, lockups, voice,
and where the brand has landed. The site reads from that record and renders it.
A brand decision MUST NOT first appear in this site's code — it is made in the
record, then rendered.

The brand's files are held apart so they can leave: `docs/BRAND.md`, `brand/`,
`scripts/build-brand-assets.mjs` and `src/web/brand.css` are the brand's;
everything else under `src/` is the site's. Page styling MUST NOT be added to
`brand.css`, and brand values MUST NOT be written anywhere but there.

**Rationale**: every snackbyte app will consume this brand, and it becomes its
own package at the second consumer. If the site owns the brand, every app
depends on the site. Keeping the boundary drawn now makes that extraction a
move rather than a rewrite.

### II. Derived, Never Hand-Cut

Anything that can be computed from a source MUST be computed from it: every
asset in `brand/` from the four geometry numbers and the palette; the favicons
and the link-preview card copied into `public/` at build; each client section's
landing page from its `section.json`. A generated file MUST NOT be hand-edited
— the generator is fixed and the file regenerated.

Generated output that only this deployment needs is produced at build and
excluded from version control. Generated output that people and other repos
consume — `brand/` — is committed, and regenerating it MUST produce no diff
unless its source changed.
...
A decision exists when it is written down: in `docs/BRAND.md` for the brand, in
this constitution for a principle, in `specs/` for a feature. Agreement reached
...
Copy is bound by `docs/BRAND.md` § Voice: say what it does, then what it
...
- `docs/BRAND.md` — the brand: tokens, geometry, lockups, voice, and the
  rollout list that tracks where the brand has landed across repositories.
- `docs/SECTIONS.md` — the client-section contract, binding on contributors
  from outside this repository.
- `brand/` and `scripts/build-brand-assets.mjs` — the cut assets and the one
  generator that produces them.
- `src/web/brand.css` — the tokens and scales as CSS; the brand's half of the
  stylesheet.
